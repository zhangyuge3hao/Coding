...
public class j_3 {
	...
	public static ArrayList<String> findExp(int[] a, final int SUM) {
		ArrayList<String> result = new ArrayList<String>(); // 存放的是所有找到的表达式
		Arrays.sort(a); // 对初始数据进行排序，便于算法的实现
		ArrayList<Integer> l = new ArrayList<Integer>(); // 存放的是当前找到的合法的元素的下标，这些元素的和小于SUM
		find(a, SUM, 0, l, result); // 通过递归的方式进行查找
		return result;
	} // 递归函数，每一次的动作很简单，
	// 在已经找到的n个元素的基础上，寻找第n+1个元素，

	public static void find(int[] a,  int SUM, int cur, ArrayList<Integer> l, ArrayList<String> result) {
		int beg = l.size() == 0 ? 0 : l.get(l.size() - 1) + 1; // 当前元素的查找范围的起始位置，返回最后一个元素值+1
